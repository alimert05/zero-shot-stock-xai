...
from weighting.event_classifier import HORIZON_LABEL_TO_CATEGORY
...
def _run_full_pipeline(
    ...
    if SENTIMENT_MODEL == "zero-shot":
        from predictors.zero_shot import run_sentiment_prediction
        preds_path = str(ZEROSHOT_PREDS)
    elif SENTIMENT_MODEL == "ProsusAI/finbert":
        from predictors.finbert import run_sentiment_prediction
        preds_path = str(FINBERT_PREDS)
    elif SENTIMENT_MODEL == "fingpt":
        from predictors.fingpt import run_sentiment_prediction
        preds_path = str(FINGPT_PREDS)
    elif SENTIMENT_MODEL in ("ollama-llama3", "ollama-mistral"):
        from predictors.ollama_predictor import run_sentiment_prediction
